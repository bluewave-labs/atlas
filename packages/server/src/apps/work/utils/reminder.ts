...
/**
 * Send reminders for work-app tasks with due dates of today.
 * Mirrors the logic in apps/tasks/reminder.ts but scoped to the work app.
 */
export async function sendDueTaskReminders(): Promise<number> {
  ...
  try {
    ...
    if (dueTasks.length === 0) {
      logger.debug('No work tasks due today need reminders');
      return 0;
    }

    logger.info({ count: dueTasks.length }, 'Found work tasks due today for reminders');
    ...
    for (const task of dueTasks) {
      try {
        ...
        reminderCount++;
        logger.info({ taskId: task.id, userId: task.userId }, 'Work task due-date reminder sent');
      } catch (err) {
        logger.warn({ err, taskId: task.id }, 'Failed to send work task reminder');
      }
    }

    logger.info({ reminderCount }, 'Work task reminder batch complete');
    return reminderCount;
  } catch (error) {
    logger.error({ error }, 'Failed to run work task due-date reminders');
    return 0;
  }
}
...
export function startWorkTaskReminderScheduler() {
  ...
  reminderTimer = setInterval(async () => {
    try {
      ...
      logger.error({ err }, 'Work task reminder scheduler failed');
    }
  }, REMINDER_INTERVAL_MS);

  setTimeout(() => sendDueTaskReminders().catch(() => {}), 90_000);

  logger.info('Work task reminder scheduler started (hourly)');
}

export function stopWorkTaskReminderScheduler() {
  if (reminderTimer) {
    clearInterval(reminderTimer);
    reminderTimer = null;
    logger.info('Work task reminder scheduler stopped');
  }
}
