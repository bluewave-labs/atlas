...
export async function getLifecycleTimeline(tenantId: string, employeeId: string) {
  return db.select().from(hrLifecycleEvents)
    .where(and(eq(hrLifecycleEvents.tenantId, tenantId), eq(hrLifecycleEvents.employeeId, employeeId), eq(hrLifecycleEvents.isArchived, false)))
    .orderBy(desc(hrLifecycleEvents.eventDate), desc(hrLifecycleEvents.createdAt));
}

export async function createLifecycleEvent(tenantId: string, input: {
  ...
  const [created] = await db.insert(hrLifecycleEvents).values({
    tenantId, employeeId: input.employeeId, eventType: input.eventType,
...
export async function deleteLifecycleEvent(tenantId: string, id: string) {
  const now = new Date();
  const [updated] = await db.update(hrLifecycleEvents)
    .set({ isArchived: true, updatedAt: now })
    .where(and(eq(hrLifecycleEvents.id, id), eq(hrLifecycleEvents.tenantId, tenantId))).returning();
  return updated || null;
}
