...
export async function seedSampleData(userId: string, accountId: string) {
  // Idempotency guard
  const existing = await db.select({ id: projectClients.id }).from(projectClients)
    .where(eq(projectClients.accountId, accountId)).limit(1);
  if (existing.length > 0) return { skipped: true };

  const now = new Date();

  // Create clients
  const [acme] = await db.insert(projectClients).values({
    accountId, userId, name: 'Acme Corporation', email: 'billing@acme.com', phone: '+1-555-1000',
    address: '100 Innovation Drive', city: 'San Francisco', state: 'CA', country: 'US', postalCode: '94105',
    currency: '$', notes: 'Enterprise client since 2024', createdAt: now, updatedAt: now,
  }).returning();

  const [brightpath] = await db.insert(projectClients).values({
    accountId, userId, name: 'BrightPath Analytics', email: 'accounts@brightpath.com', phone: '+1-555-2000',
    address: '250 Data Lane', city: 'Austin', state: 'TX', country: 'US', postalCode: '78701',
    currency: '$', notes: 'Mid-market analytics company', createdAt: now, updatedAt: now,
  }).returning();

  const [nordiq] = await db.insert(projectClients).values({
    accountId, userId, name: 'Nordiq Systems', email: 'finance@nordiqsystems.se', phone: '+46-70-555-0100',
    address: 'Storgatan 12', city: 'Stockholm', state: '', country: 'SE', postalCode: '111 23',
    currency: '€', notes: 'European expansion project', createdAt: now, updatedAt: now,
  }).returning();

  // Create projects
  const [webApp] = await db.insert(projectProjects).values({
    accountId, userId, clientId: acme.id, name: 'Web Application Redesign', description: 'Complete redesign of the customer portal with React + TypeScript',
    billable: true, status: 'active', estimatedHours: 320, estimatedAmount: 48000, color: '#3b82f6',
    startDate: new Date('2026-01-15'), endDate: new Date('2026-06-30'), createdAt: now, updatedAt: now,
  }).returning();

  const [mobileApp] = await db.insert(projectProjects).values({
    accountId, userId, clientId: acme.id, name: 'Mobile App MVP', description: 'React Native mobile app for iOS and Android',
    billable: true, status: 'active', estimatedHours: 200, estimatedAmount: 30000, color: '#8b5cf6',
    startDate: new Date('2026-02-01'), endDate: new Date('2026-05-31'), createdAt: now, updatedAt: now,
  }).returning();

  const [dashboard] = await db.insert(projectProjects).values({
    accountId, userId, clientId: brightpath.id, name: 'Analytics Dashboard', description: 'Real-time data visualization dashboard with D3.js',
    billable: true, status: 'active', estimatedHours: 150, estimatedAmount: 22500, color: '#10b981',
    startDate: new Date('2026-03-01'), endDate: new Date('2026-07-31'), createdAt: now, updatedAt: now,
  }).returning();

  const [apiInteg] = await db.insert(projectProjects).values({
    accountId, userId, clientId: nordiq.id, name: 'API Integration Layer', description: 'REST API integration with third-party shipping providers',
    billable: true, status: 'active', estimatedHours: 80, estimatedAmount: 12000, color: '#f59e0b',
    startDate: new Date('2026-03-15'), endDate: new Date('2026-05-15'), createdAt: now, updatedAt: now,
  }).returning();

  await db.insert(projectProjects).values({
    accountId, userId, clientId: brightpath.id, name: 'Internal DevOps Setup', description: 'CI/CD pipeline and monitoring infrastructure',
    billable: false, status: 'completed', estimatedHours: 40, color: '#6b7280',
    startDate: new Date('2026-01-01'), endDate: new Date('2026-02-28'), createdAt: now, updatedAt: now,
  });

  // Add project members (current user to all projects)
  for (const proj of [webApp, mobileApp, dashboard, apiInteg]) {
    await db.insert(projectMembers).values({
      userId, projectId: proj.id, hourlyRate: 150, role: 'manager', createdAt: now, updatedAt: now,
    });
  }

  // Create time entries for the past 2 weeks
  const tasks = [
    { desc: 'Frontend development', task: 'Development' },
    { desc: 'Code review and testing', task: 'Review' },
    { desc: 'Client meeting and requirements', task: 'Meetings' },
    { desc: 'Architecture planning', task: 'Planning' },
    { desc: 'Bug fixes and QA', task: 'Bug fixes' },
    { desc: 'Documentation', task: 'Documentation' },
  ];

  const projects = [webApp, mobileApp, dashboard, apiInteg];
  ...
  for (let dayOffset = -13; dayOffset <= 0; dayOffset++) {
    ...
    // 2-3 entries per day across different projects
    ...
    for (let i = 0; i < numEntries; i++) {
      ...
      await db.insert(projectTimeEntries).values({
        accountId, userId, projectId: proj.id,
        durationMinutes: duration,
        workDate: dateStr,
        billable: true, billed: false, locked: false,
        notes: taskInfo.desc, taskDescription: taskInfo.task,
        createdAt: now, updatedAt: now,
      });
    }
  }

  // Create a sent invoice for Acme
  const [inv1] = await db.insert(projectInvoices).values({
    accountId, userId, clientId: acme.id,
    invoiceNumber: 'INV-001', status: 'sent',
    amount: 7500, tax: 0, taxAmount: 0, discount: 0, discountAmount: 0,
    currency: '$', issueDate: new Date('2026-03-15'), dueDate: new Date('2026-04-15'),
    notes: 'Web Application Redesign — March milestone', sentAt: new Date('2026-03-15'),
    createdAt: now, updatedAt: now,
  }).returning();

  await db.insert(projectInvoiceLineItems).values([
    { invoiceId: inv1.id, description: 'Frontend development — 20 hours', quantity: 20, unitPrice: 150, amount: 3000, createdAt: now, updatedAt: now },
    { invoiceId: inv1.id, description: 'Architecture planning — 10 hours', quantity: 10, unitPrice: 150, amount: 1500, createdAt: now, updatedAt: now },
    { invoiceId: inv1.id, description: 'Code review and QA — 12 hours', quantity: 12, unitPrice: 150, amount: 1800, createdAt: now, updatedAt: now },
    { invoiceId: inv1.id, description: 'Client meetings — 8 hours', quantity: 8, unitPrice: 150, amount: 1200, createdAt: now, updatedAt: now },
  ]);

  // Create a paid invoice for BrightPath
  await db.insert(projectInvoices).values({
    accountId, userId, clientId: brightpath.id,
    invoiceNumber: 'INV-002', status: 'paid',
    amount: 4500, tax: 0, taxAmount: 0, discount: 0, discountAmount: 0,
    currency: '$', issueDate: new Date('2026-02-28'), dueDate: new Date('2026-03-28'),
    notes: 'Analytics Dashboard — February work', sentAt: new Date('2026-02-28'), paidAt: new Date('2026-03-10'),
    createdAt: now, updatedAt: now,
  });

  // Create a draft invoice
  await db.insert(projectInvoices).values({
    accountId, userId, clientId: nordiq.id,
    invoiceNumber: 'INV-003', status: 'draft',
    amount: 3600, tax: 10, taxAmount: 360, discount: 0, discountAmount: 0,
    currency: '€', issueDate: new Date('2026-03-30'), dueDate: new Date('2026-04-30'),
    notes: 'API Integration — Phase 1',
    createdAt: now, updatedAt: now,
  });

  // Create settings
  await db.insert(projectSettings).values({
    accountId, invoicePrefix: 'INV', defaultHourlyRate: 150,
    companyName: 'Atlas Studio', companyAddress: '123 Main St, San Francisco, CA 94105',
    nextInvoiceNumber: 4, createdAt: now, updatedAt: now,
  }).onConflictDoNothing();

  logger.info({ userId, accountId }, 'Seeded Projects sample data');
  return { clients: 3, projects: 5, invoices: 3, timeEntries: '~30' };
}
